...
# Host-side runner. Exact graph checks run in a disposable container.
require 'digest'
require 'fileutils'
require 'json'
require 'open3'
...
options = { docker: 'docker', timeout: 1800, perf_reps: 5 }
parser = OptionParser.new do |p|
  p.banner = 'Usage: ruby bin/woods_reference_acceptance.rb (--woods DIR | --gem FILE --sha256 DIGEST) --revision SHA [options]'
  ...
  p.on('--baseline-woods DIR', 'Optional committed source baseline for alternating performance trials') { |v| options[:baseline] = File.expand_path(v) }
...
begin
  ...
  if options[:baseline]
    ...
    raise "baseline revision mismatch: #{actual}" unless actual == options[:baseline_revision]
  elsif options[:baseline_revision]
    raise OptionParser::InvalidArgument, '--baseline-revision requires --baseline-woods'
  end
  if options[:gem]
...
report = options[:report_dir] || Dir.mktmpdir('woods-reference-evidence-')
...
run_id = "woods-reference-#{SecureRandom.hex(6)}"
...
evidence[:performance_baseline] = { revision: options[:baseline_revision], repetitions: options[:perf_reps],
                                  tested_content: 'committed snapshot; working-tree edits excluded' } if options[:baseline]
if options[:woods]
...
image_created = false
exit_code = 1
puts "Source-reference acceptance (#{mode}); evidence: #{report}"

begin
  Dir.mktmpdir('woods-reference-run-') do |scratch|
    ...
    if options[:baseline]
      baseline_archive = File.join(scratch, 'baseline.tar')
      baseline_snapshot = File.join(scratch, 'baseline')
      FileUtils.mkdir_p(baseline_snapshot)
      command!(['git', '-C', options[:baseline], 'archive', '--format=tar', "--output=#{baseline_archive}", options[:baseline_revision]], log)
      command!(['tar', '-xf', baseline_archive, '-C', baseline_snapshot], log)
      evidence[:performance_baseline][:archive_sha256] = Digest::SHA256.file(baseline_archive).hexdigest
    end
    ...
    File.write(bootstrap, <<~'RUBY')
      ...
      if ENV['WOODS_REFERENCE_BASELINE_GEMFILE']
        baseline = ENV.fetch('WOODS_REFERENCE_BASELINE_GEMFILE')
        # Reuse exact non-Woods dependency versions from the candidate bundle.
        gemfile = File.read('Gemfile').sub(/^gem "woods".*$/, 'gem "woods", path: "/woods-baseline"')
        File.write(baseline, gemfile)
        if ENV.fetch('WOODS_ACCEPTANCE_MODE') == 'source'
          File.write("#{baseline}.lock", File.read('Gemfile.lock').sub('/woods-gem', '/woods-baseline'))
        end
        abort 'baseline bundle install failed' unless system({ 'BUNDLE_GEMFILE' => baseline, 'BUNDLE_APP_CONFIG' => '/app/.bundle-baseline' }, 'bundle', 'install', '--jobs', '4', '--retry', '2')
      end
      exec({ 'BUNDLE_GEMFILE' => '/app/Gemfile' }, 'bundle', 'exec', 'ruby', '/harness/tools/woods_reference_acceptance.rb')
    RUBY
    ...
    if options[:baseline]
      mounts << [baseline_snapshot, '/woods-baseline']
      env['WOODS_REFERENCE_BASELINE_GEMFILE'] = '/app/Gemfile.baseline'
      env['WOODS_REFERENCE_BASELINE_REVISION'] = options[:baseline_revision]
      env['WOODS_REFERENCE_PERF_REPS'] = options[:perf_reps].to_s
    end
